    ...
    <main className={`page-shell ${compact ? "page-shell-compact" : ""} ${className}`}>
      ...
      <header className="masthead">
        ...
        <nav className="desktop-nav" aria-label="Primary navigation">
          {links.map((link) => <Link className={`nav-link ${active === link.section ? "is-active" : ""}`} href={link.href} key={link.section}>{link.label}</Link>)}
        </nav>
      ...
      <nav className="mobile-nav" aria-label="Mobile navigation">
        {links.map((link) => <Link className={`mobile-nav-link ${active === link.section ? "active" : ""}`} href={link.href} key={link.section}>{link.label}</Link>)}
      </nav>
    </main>
  );
}
